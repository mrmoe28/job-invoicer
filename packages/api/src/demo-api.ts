// Temporary demo data implementation for CRUD operations
// This provides working CRUD functionality while database dependencies are resolved
...
export const appRouter = router({
  ...
  getTasks: publicProcedure
    ...
    .query(({ input }) => {
      const filters: any = {};
      if (input.jobId) filters.jobId = input.jobId;
      if (input.status) filters.status = input.status;
      
      const tasks = findByOrgId(demoData.tasks, filters);
      return tasks.map(task => {
        const job = findById(demoData.jobs, task.jobId);
        return {
          ...task,
  ...
  createTask: publicProcedure
    ...
    .mutation(({ input }) => {
      const task = createItem(demoData.tasks, {
        ...input,
        status: 'pending',
  ...
  updateTask: publicProcedure
    ...
    .mutation(({ input }) => {
      ...
      return updateById(demoData.tasks, id, updateData);
    }),

  deleteTask: publicProcedure
    .input(z.object({ id: z.string() }))
    .mutation(({ input }) => {
      return updateById(demoData.tasks, input.id, { isActive: false });
    }),
  ...
  getDocuments: publicProcedure
    ...
    .query(({ input }) => {
      const filters: any = {};
      if (input.jobId) filters.jobId = input.jobId;
      if (input.fileType) filters.fileType = input.fileType;
      if (input.category) filters.category = input.category;
      
      const documents = findByOrgId(demoData.documents, filters);
      return documents.map(doc => {
        const job = doc.jobId ? findById(demoData.jobs, doc.jobId) : null;
        return {
          ...doc,
  ...
  createDocument: publicProcedure
    ...
    .mutation(({ input }) => {
      const document = createItem(demoData.documents, {
        ...input,
        uploadedByUserId: 'user-1',
        status: 'active',
        isPublic: false,
  ...
  updateDocument: publicProcedure
    ...
    .mutation(({ input }) => {
      const { id, ...updates } = input;
      return updateById(demoData.documents, id, updates);
    }),

  deleteDocument: publicProcedure
    .input(z.object({ id: z.string() }))
    .mutation(({ input }) => {
      return updateById(demoData.documents, input.id, { status: 'deleted' });
    }),
  ...
  getCrewMembers: publicProcedure
    ...
    .query(({ input }) => {
      ...
      return findByOrgId(demoData.crewMembers, filters).map(member => ({
        ...member,
  ...
  updateCrewMember: publicProcedure
    ...
    .mutation(({ input }) => {
      const { id, ...updates } = input;
      return updateById(demoData.crewMembers, id, updates);
    }),

  deleteCrewMember: publicProcedure
    .input(z.object({ id: z.string() }))
    .mutation(({ input }) => {
      return updateById(demoData.crewMembers, input.id, { isActive: false });
    }),
});
